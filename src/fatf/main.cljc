(ns fatf.main
  "Kotodama WASM entrypoint for the Fatf clean-room actor (L4) — Clojure port.
  ...
  facts (`fatf.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"fatf.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fatf-compat" :tier tier :entities entities} 200])
